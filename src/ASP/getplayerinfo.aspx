...
else
{
	...
	if(strpos($info,$requiredKeys) !== false)
	{
		...
		if(strpos($info, 'mng-') !== false)
		{
			...
			{
				$query = "SELECT * FROM `mapinfo` WHERE `id` >= ". Config::Get('game_custom_mapid');
				$resultm = $connection->query($query);
				if($resultm instanceof PDOStatement && $resultm->rowCount() > 0)
				{
					$usermaps = array();
					while ($rowum = $resultm->fetch()) 
						$usermaps[] = $rowum['id'];
		...
		else 
		{
			...
			$result = $connection->query($query);
			if($result instanceof PDOStatement && ($row2 = $result->fetch()))
			{
				$favoi = $row2['attacker'];
				$favok = $row2['count'];
			...
			$row2 = $result->fetch();
			arsort($row2);
			$favk = str_replace("time", "", key($row2));
			...
			{
				arsort($row2);
				$favv = str_replace("time", "", key($row2));
			}
			...
			// Favorite weapon 
			$query = "SELECT `time0`, `time1`, `time2`, `time3`, `time4`, `time5`, `time6`, `time7`, `time8`, `knifetime`, `shockpadtime`, (`c4time`+`claymoretime`+`atminetime`) AS `exptime`, `handgrenadetime` FROM `weapons` WHERE `id` = {$pid}";
			$result = $connection->query($query);
			$row2 = $result->fetch();
			arsort($row2);
			$favw = str_replace("time", "", key($row2));
			if (!is_numeric($favw))
			{
				switch($favw)
				{
			...
			if (strpos($info, 'mng-') !== false)
			{
				...
				$resultm = $connection->query($querym);
				if($resultm instanceof PDOStatement)
				{
	...
	elseif ($info == 'mtm-,mwn-,mls-' || $info == 'mtm-,mwn-,mls-,mbs-,mws-' || $info == 'mtm-,mwn-,mls-,cmap-' || $info == 'mtm-,mwn-,mls-,mbs-,mws-,cmap-')
	{
		...
		if(strpos($info, 'cmap-') !== false)
		{
			$query = "SELECT `id` FROM `mapinfo` WHERE `id` >= ". Config::Get('game_custom_mapid') ." ORDER BY `id`";
			$resultm = $connection->query($query);
			if($resultm instanceof PDOStatement && ($rowum = $resultm->fetchColumn()))
			{
				$usermaps = array();
				do {
					$usermaps[] = $rowum;
				}
				while($rowum = $resultm->fetchColumn());
	...
	elseif ($info == 'rank')
	{
		...
		else
		{
			...
			$num += strlen(preg_replace('/[\t\n]/','',$head));
			$out .= "D\t$row[id]\t$row[name]\t$row[rank]\t$row[chng]\t$row[decr]\n";
	...
	elseif (checkGameServerRequest($info))
	{
		...
		if(!($result instanceof PDOStatement) || !($roww = $result->fetch()))  
		{
			...
			$roww['tacticaldeployed'] = $roww['grapplinghookdeployed'] = $roww['ziplinedeployed'] = '0';
		}
